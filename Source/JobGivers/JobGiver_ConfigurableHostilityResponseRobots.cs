...
namespace CrimsonGridFramework.JobGivers
{
    public class JobGiver_ConfigurableHostilityResponseRobots : JobGiver_ConfigurableHostilityResponse
    {
        ...
        private Job TryGetAttackNearbyEnemyJob(Pawn pawn)
        {
            ...
            Verb verb = pawn.TryGetAttackVerb(thing, !pawn.IsColonist, true);
            if (verb == null || verb.ApparelPreventsShooting())
            {
                return null;
            }
            Log.Message($"GETTING VERB FROM: {verb.EquipmentSource.def.defName}");
